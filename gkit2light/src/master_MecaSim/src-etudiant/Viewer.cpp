...
int Viewer::render( )
{
    ...
    for(e=_Simu->_enfants.begin(); e!=_Simu->_enfants.end(); e++)
    {
       ...
       // gl.draw(m_plan);
        ...
        gl.model(Translation(-1.05,0.9,1.05) * Scale(0.5,0.5,0.5));
        gl.draw(m_sphere);
        // Passage a l objet suivant

        // gl.model(Translation(-10,0,10) * Scale(0.1,0.1,0.1)*RotationX(180));
        gl.model(Translation(-10,-0.4,-10) * Scale(0.1,0.1,0.1));
        gl.texture(m_terrain_texture);
        gl.draw(m_terrain);
        
       /* gl.texture(0);
        gl.model(Translation(0,1,-1) * Scale(0.1,0.1,0.1));
        gl.draw(m_bigguy);*/
        
        gl.texture(m_arbre_texture);
        draw_foret(Translation(-11,-0.5,-10)*Scale(0.1,0.1,0.1));
        
        
        gl.texture(m_column_texture);
        draw_ruines(Translation(-10,-0.5,-10)*Scale(0.1,0.1,0.1));
